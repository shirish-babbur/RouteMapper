...
/***
 * Simple BackWard command which updates the x and y co ordinates based on how much
 * units distance to go back to at current angle plane.
 * @param units: Int
 */
case class Backward(units: Int) extends Command {

  /***
   * Basic apply function to apply backward motion by units distance
