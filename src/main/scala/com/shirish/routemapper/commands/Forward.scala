...
/***
 * Simple Forward command which updates the x and y co ordinates based on how much
 * units distance to go forward to at current angle plane.
 * @param units: Int
 */
case class Forward(units: Int) extends Command {

  /***
   * Basic apply function to apply forward motion by units distance
