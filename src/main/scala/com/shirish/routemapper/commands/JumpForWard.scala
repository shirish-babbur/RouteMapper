...
/***
 * Simple Jump Forward command which updates the x and y co ordinates based on how much
 * units distance to jump forward to at current angle plane.
 * @param units: Int
 */
case class JumpForWard(units: Int) extends Command {

  /***
   * Basic apply function to apply jump forward motion by units distance
